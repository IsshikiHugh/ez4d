...
def replace_nodes_name_prefix(
    ...
    """ 
    INPLACE! 
    Replace the prefix of the keys in the state_dict with the new prefix.
    """
...
def match_prefix_and_remove_nodes(
    state_dict : Dict[str, object],
    prefix     : str,
):
    """ 
    ...
    Opposite of `match_prefix_and_remove_other_nodes`. 
    """
...
def match_prefix_and_remove_other_nodes(
    state_dict : Dict[str, object],
    prefix     : str,
):
    """ 
    INPLACE! 
    Remove the keys in the state_dict that does NOT start with the prefix. 
    Opposite of `match_prefix_and_remove_nodes`. 
    """
    for name in list(state_dict.keys()):
...
def match_prefix_and_return_state_dict_with_new_prefix(
    state_dict   : Dict[str, object],
    match_prefix : str,
    new_prefix   : str,
):
    """ 
    INPLACE! 
    Return a new state_dict with the keys that starts with the `match_prefix`,
    but the prefix will be replaced with the `new_prefix`.
    """
    state_dict = match_prefix_and_remove_other_nodes(state_dict, match_prefix)
    state_dict = replace_nodes_name_prefix(state_dict, match_prefix, new_prefix)
    return state_dict
