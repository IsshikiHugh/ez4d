...
from .info.log import get_logger
...
def lurb_to_cwh(
    ...
    """
    lurb, recover_type_back = to_torch(lurb, device=None, temporary=True)
...
def cwh_to_lurb(
    ...
    - Union[list, np.ndarray, torch.Tensor], (..., 4)
        - The left-upper-right-bottom format bounding box.
    """
    cwh, recover_type_back = to_torch(cwh, device=None, temporary=True)
...
def cs_to_cwh(
    ...
    - Union[list, np.ndarray, torch.Tensor], (..., 4)
        - The center-width-height format bounding box.
    """
    cs, recover_type_back = to_torch(cs, device=None, temporary=True)
...
def luwh_to_lurb(
    ...
    - Union[list, np.ndarray, torch.Tensor]
        - The left-upper-right-bottom format bounding box.
    """
    luwh, recover_type_back = to_torch(luwh, device=None, temporary=True)
...
def crop_with_lurb(data, lurb, padding=0):
    ...
    """
    data, recover_type_back = to_torch(data, device=None, temporary=True)
