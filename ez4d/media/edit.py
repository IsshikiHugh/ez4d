...
def pad_img(
    ...
    - align: str, default 'c-c'
        - The alignment of the frames. It should be in the format of 'h-v', 
          where 'h' and 'v' can be 'l', 'c', 'r', referring to left(top), center, right(bottom) respectively.
...
def rgb_to_grayscale(rgb_frames: np.ndarray) -> np.ndarray:
    """
    Convert the RGB frames to grayscale.

    ### Args
    - frames: np.ndarray, (L, H, W, 3) or (H, W, 3)
        - The input RGB frame(s).

    ### Returns
    - np.ndarray, (L, H, W, 1) or (H, W, 1)
        - The output grayscale frame(s).
    """
    assert len(rgb_frames.shape) in [3, 4], 'rgb_frames must have 3 or 4 dimensions.'
    if len(rgb_frames.shape) == 3:
        rgb_frames = rgb_frames[None]
        squeezed = True
    else:
        squeezed = False

    gray_frames = np.stack([cv2.cvtColor(frame, cv2.COLOR_RGB2GRAY)[..., None] for frame in rgb_frames], axis=0)  # (L, H, W, 1)
    gray_frames = gray_frames.astype(np.uint8)  # (L, H, W, 1)

    if squeezed:
        gray_frames = gray_frames.squeeze(0)
    return gray_frames
