...
def get_global_cameras_following(verts, device="cuda", offsets=(-5.0, 5.0, 5.0), distance=5):
    """This always put object at the center of view"""
    offsets = torch.tensor([offsets]).repeat(len(verts), 1).to(device)
    ...
    rotation = look_at_rotation(positions, targets).mT.to(device)
    translation = -(rotation @ positions.unsqueeze(-1)).squeeze(-1)

    lights = [PointLights(device=device, location=[position]) for position in positions.cpu().numpy()]
    return rotation, translation, lights


def get_global_cameras_staring(verts, device="cuda", position=(-5.0, 5.0, 0.0), distance=5):
    """This always put object at the center of view"""
    ...
    lights = [PointLights(device=device, location=[position])] * len(verts)
    return rotation, translation, lights
